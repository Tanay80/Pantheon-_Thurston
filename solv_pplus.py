...
def integralFunc(z, A, h, sigma, e2, om_k, alpha):

    A = (1 / (1+z)) * ((np.abs(1 - e2 * np.cos(alpha) * np.cos(alpha)) ** (1/2)) / (np.abs(1 - e2) ** (1/3))) 
    dAdz = ((A*A) * (np.abs(1-e2) ** (1/3)) * (np.abs(1 - e2 * np.cos(alpha) * np.cos(alpha)) ** (1/2))) / ((sigma + 1) * e2 * np.cos(alpha) * np.cos(alpha) + (2 - 3 * np.cos(alpha) * np.cos(alpha)) * sigma - 1)
                                      
    G = (1 - e2) ** (1/6)
    H = np.sqrt(1 - e2 * np.cos(alpha) * np.cos(alpha))
    f = G / H          
...
def my_likelihood(h, sigma, e2, om_im, om_k, la, ba, M0, wbar):   
    ...
    for i in range(len(z)):
        ...
        I0 = integralFunc(z_, par.y.T[:, 0], par.y.T[:, 1], par.y.T[:, 2], par.y.T[:, 3], par.y.T[:, 4], alpha[i])
        I = -romb(I0, dx=dz) 
